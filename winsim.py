...
def parse_args():
    parser = argparse.ArgumentParser(description='Download winSIM invoices.')
    parser.add_argument('--all', action='store_true', help='Download all invoices, even if already downloaded')
...
def main():
    ...
    with requests.Session() as session:
        ...
        if 'rechnungen' not in resp.text.lower():
        ...
        for link in invoice_links:
            ...
            if file_from_header:
                ...
                print('Could not determine filename for invoice, skipping.')
                continue
            if os.path.exists(filename) and not args.all:
                print(f'Skipping already downloaded invoice: {filename}')
                break
            elif os.path.exists(filename) and args.all:
                print(f'Skipping already downloaded invoice: {filename}')
                continue
